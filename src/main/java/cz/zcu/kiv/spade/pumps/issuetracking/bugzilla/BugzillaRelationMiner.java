    ...
    protected void mineRelations() {
        for (WorkUnit unit : pump.getPi().getProject().getUnits()) {
            ...
            try {
                ...
                App.printLogMsg(this, String.format(ISSUE_NOT_EXISTS_ERR_FORMAT, unit.getExternalId()));
                continue;
            }
            if (issues != null) {
                for (Issue issue : issues) {
                    ...
                    if (issue.getChildCount() > 0) {
                        for (Issue childIssue : issue.getChildren()) {
                            ...
                            App.printLogMsg(this, String.format(LINK_FORMAT, unit.getNumber(), "parent of", child.getNumber()));
    ...
    private Relation resolveRelation(IssueLink link) {
        ...
        for (Relation relation : pump.getPi().getRelations()) {
            if (toLetterOnlyLowerCase(link.getLinkType().name()).equals(toLetterOnlyLowerCase(relation.getName()))) {
                Relation newRelation = new Relation(link.getLinkTypeName(), new RelationClassification(relation.getaClass()));
        ...
        Relation newRelation = new Relation(link.getLinkTypeName(), new RelationClassification(aClass));
        pump.getPi().getRelations().add(newRelation);
        return newRelation;
    }
}
